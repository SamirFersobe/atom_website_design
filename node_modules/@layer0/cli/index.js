...
if (majorVersion !== supportedMajorVersion) {
  ...
  if (majorVersion < supportedMajorVersion) {
    console.log(`Layer0${cliVersion} supports only Node.js v${supportedMajorVersion} but your current version is ${process.version}.`);
    ...
    console.log(`WARNING: Layer0${cliVersion} platform serverless runtime supports only Node.js v${supportedMajorVersion}. Your current version is ${process.version}.`);
    console.log(`If your backend code (SSR, Api endpoints, etc.) uses Node.js ${process.version} features it may not work as expected once deployed on the Layer0 platform.`);
  }
