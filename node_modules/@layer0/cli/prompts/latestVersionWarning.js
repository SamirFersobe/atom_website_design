...
const CHANGELOG_URL = 'https://docs.layer0.co/guides/changelog';
...
module.exports = async ({
  ...
  try {
    ...
    const currentVersionRange = packageJson.findCurrentLayer0Version();
    const cmdOutput = await getNodePackageInfo('@layer0/cli');
    ...
    if (hasNewVersion) {
      logger.warn('⚠️', ` A new version of Layer0 is available: ${chalk.green(`v${latestVersion}`)}.`);
      logger.warn(`Run "${chalk.green('layer0 use latest')}" to upgrade your app.`);
