...
var _Layer0PackageJson = _interopRequireDefault(require("./Layer0PackageJson"));
...
function isMonorepo() {
  ...
  const workspaces = _Layer0PackageJson.default.loadPackageJson(process.cwd()).workspaces;
...
async function locateAppToRunCmd(action) {
  const appPaths = (await getAppPaths()).filter(({
    path
  }) => (0, _fs.existsSync)(`${path}/layer0.config.js`));
